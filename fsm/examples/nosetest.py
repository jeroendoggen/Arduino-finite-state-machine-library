...
#!/usr/bin/env python
#
# nose_test.py is copyright 2013,2014 Jeroen Doggen.
...
def test_build_examples():
    ...
    for example in os.listdir(path):
        if(os.path.isdir(example)):
            shutil.copy("SConstruct", example)
            os.chdir(example)
...
def cleanup(example):
    ...
    hexfile = example + ".hex"
    safe_remove(example + ".hex")

    elffile = example + ".elf"
    safe_remove(example + ".elf")

    sconsfile = ".sconsign.dblite"
    safe_remove(".sconsign.dblite")

    SConstruct = "SConstruct"
    safe_remove("SConstruct")
